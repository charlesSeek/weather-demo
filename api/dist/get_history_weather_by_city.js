...
const handler = async (event) => {
    ...
    let dt = ((_b = event.queryStringParameters) === null || _b === void 0 ? void 0 : _b.dt) ? parseFloat(event.queryStringParameters.dt) : Math.floor(Date.now() / 1000);
    if (isNaN(dt)) {
    ...
    try {
        ...
        const weather = await (0, utils_1.getHistoryWeather)(coordinate, dt, apiKey);
        if (!weather) {
            return {
                statusCode: 500,
                headers: {
